...
export function assembleQuiz(bankRows, subjectId, grade, count, difficulty = null, track = null, recent = null) {
  ...
  const bankQuestions = preferUnseen(shuffle(usableBank), recent, normalizeBankRow)
    .slice(0, bankTarget).map(normalizeBankRow);
 ...
 *
 * Proportional rather than a point per correct answer, because a drill is sized
 * off the error count — `min(20, max(5, wrongCount * 2))` — and so almost never
 * matches the hole it is filling. Crediting per answer would let a 20-question
 * drill close a 14-point gap with six answers to spare, which makes the last
 * six free; and a 5-question drill could never close a 14-point gap at all.
 *
