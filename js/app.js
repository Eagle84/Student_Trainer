...
import { DEFAULT_SUBJECT, topicsFor, subjectsForGrade, gradelessSubjects, NO_GRADE } from './subjects/index.js';
...
function saveMessage(outcome, credited) {
  ...
  return `הציון במבחן המקורי עודכן ל-${outcome.score} מתוך ${outcome.total_questions} (${Math.round(outcome.percent)}%).`;
}

function handleConfirmRestart() {
  if (window.confirm('האם אתה בטוח שברצונך להפסיק את המבחן ולחזור למסך ההגדרות?')) {
    // Abandoning is a decision, so this quiz is not kept for later. Leaving it
    // would greet the student with an offer to resume the thing they just quit;
    // any others they have parked are untouched.
    if (quiz) db.clearActiveQuiz(quiz.startedAt);
    quiz = null;
    lastAnswerIndex = null;
    creditTarget = null;
    ui.showScreen('setup-screen');
...
async function handlePracticeAttempt(button) {
  ...
  const count = Math.min(20, Math.max(5, errors.length * 2));
  const [bankRows, recent] = await Promise.all([
...
async function handlePracticeWeak() {
  ...
  const count = Math.min(20, Math.max(5, wrongCount * 2));
...
function currentScope() {
  if (!studyScope) {
    studyScope = { grade: Number(ui.getGrade()), subjectId: ui.getSubjectId(), track: ui.getTrack() };
  }
  return studyScope;
}
...
async function refreshScopeSurface() {
  ...
  if (scopeHost === 'setup') {
    ...
    uiLesson.renderScopePicker(document.getElementById('drawer-scope'), scope, { host: 'drawer' });
    drawer.renderDrawer(scope.subjectId, scope.grade, scope.track, new Map());
    drawer.renderDrawer(scope.subjectId, scope.grade, scope.track, await db.getLessonProgress());
  } else if (scopeHost === 'learn') {
    uiLesson.renderScopePicker(document.getElementById('learn-scope'), scope);
    const topics = topicsFor(scope.subjectId, scope.grade, scope.track);
    uiLesson.renderCatalog(scope.subjectId, scope.grade, topics, new Map());
    uiLesson.renderCatalog(scope.subjectId, scope.grade, topics, await db.getLessonProgress());
  }
}
...
function handleStudySubject(button) {
  const scope = currentScope();
...
document.addEventListener('click', event => {
  ...
  switch (action) {
    ...
    case 'study-subject':    handleStudySubject(actionEl); break;
