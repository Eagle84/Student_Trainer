...
/**
 * The subject id a quiz drawing on several subjects is recorded under.
 *
 * Not a real subject: it has no generators and never appears in a picker. It
 * exists so one mixed sitting is one attempt with one score, instead of three
 * rows pretending to be three quizzes.
 *
 * Nothing may look up generators by it. `attempt_errors` carries each mistake's
 * own subject precisely so weak-topic drilling never has to.
 */
export const MIXED = 'mixed';

/** True for the pseudo-subject a mixed quiz is recorded under. */
export function isMixed(id) {
  return id === MIXED;
}
