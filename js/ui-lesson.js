...
import { getSubject, subjectsForGrade, allGrades, SUBJECTS, isGradeless } from './subjects/index.js';
 ...
 */
export function renderScopePicker(host, scope, { host: hostName = 'learn' } = {}) {
  if (!host) return;
  ...
  const chips = el('div', { class: 'scope-subjects', attrs: { role: 'group', 'aria-label': 'בחר מקצוע' } });
  for (const id of available) {
    const chip = el('button', {
      class: 'scope-chip',
      text: SUBJECTS[id] ? SUBJECTS[id].name : id,
      attrs: { type: 'button', 'aria-pressed': String(id === scope.subjectId) }
    });
    chip.dataset.action = 'study-subject';
