...
export async function getStudentErrorTopics(name) {
  ...
  try {
    ...
    const { data, error } = await c
      ...
      .select('topic, attempt_id, attempts!inner(user_id, subject, grade, created_at)')
    ...
    return (data || []).map(r => ({
      ...
      subject: r.attempts ? r.attempts.subject : undefined,
      grade: r.attempts ? r.attempts.grade : undefined
    }));
  } catch {
    online = false;
    return [];
