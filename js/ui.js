 ...
 */
import { SUBJECTS, MIXED, subjectIds, getSubject, gradesFor, usesTracks, DEFAULT_SUBJECT, NO_GRADE, gradelessSubjects, isGradeless } from './subjects/index.js';
...
export function subjectName(id) {
  // Not in SUBJECTS — it is the pseudo-subject a mixed quiz is recorded under,
  // and it has to read as something in the history, the leaderboard and the
  // dashboard rather than falling through as the raw id.
  if (id === MIXED) return 'מעורב';
 ...
 */
/**
 * Whether the running quiz spans several subjects.
 *
 * A module-level flag for the same reason optionsDirection is one: it changes
 * once per quiz, not once per question, and threading it through every
 * renderQuestion call would put it in the signature of code that does not care.
 */
let mixedMode = false;

export function setMixedMode(on) {
  mixedMode = Boolean(on);
}
...
export function renderQuestion(question, index, total, answered) {
  // A mixed quiz interleaves subjects that disagree about direction, so it is
  // applied per QUESTION rather than once per quiz. For a single-subject quiz
  // this is the same value every time and changes nothing.
  if (question.subject) applySubjectDirection(question.subject);

  $('question-counter').textContent = `שאלה ${index + 1} מתוך ${total}`;
  // In a mixed quiz the topic alone is ambiguous — "ניגודים" exists in both
  // עברית and אנגלית, and a student needs to know which language to answer in.
  $('topic-badge').textContent = mixedMode && question.subject
    ? `${subjectName(question.subject)} · ${question.topic}`
    : `נושא: ${question.topic}`;
...
export function renderSummary(result, errors, saveMessage) {
  ...
  $('final-score').textContent =
    `${Math.round(result.score)} מתוך ${result.total_questions} (${Math.round(result.percent)}%)`;
...
export function renderLeaderboard(rows) {
  ...
  rows.forEach((row, i) => {
    ...
    score.dir = 'ltr';
    score.textContent = `${Math.round(row.score)} / ${row.total_questions}`;
...
function insightsCard(attempts, now = Date.now()) {
  ...
  if (imp) {
    ...
    tiles.push(insight(
      `${up && !flat ? '+' : ''}${flat ? '≈' : Math.round(imp.delta)}${flat ? '' : '%'}`,
      flat ? 'יציב' : (up ? 'שיפור' : 'ירידה'),
      `${imp.sampleSize} אחרונים (${Math.round(imp.recent)}%) מול ${imp.comparedWith} שלפניהם (${Math.round(imp.earlier)}%)`,
      flat ? '' : (up ? 'good' : 'bad')));
  }
...
// --- "מה חדש" -----------------------------------------------------------
// A one-time popup announcing a release, aimed at the student reading it, not
// at whoever shipped it. See js/whatsnew.js for the content rules and
// js/app.js for the decision of WHEN to show it — this module only knows how
// to paint and animate the dialog once told to.

/** Fills the dialog from one entry of js/whatsnew.js's WHATS_NEW list. */
export function renderWhatsNew(entry) {
  const emoji = $('whatsnew-emoji');
  const title = $('whatsnew-title');
  const list = $('whatsnew-list');
  if (!emoji || !title || !list) return;

  emoji.textContent = entry.emoji;
  title.textContent = entry.title;
  list.replaceChildren(...entry.items.map(item =>
    el('li', { class: 'whatsnew-item' },
      el('span', { class: 'whatsnew-item-emoji', attrs: { 'aria-hidden': 'true' }, text: item.emoji }),
      el('span', { class: 'whatsnew-item-text', text: item.text }))));
}

/**
 * Drops the panel in. Same choreography as openDrawer: unhide on both layers
 * first, then add the `.open` class on the next frame so the browser has
 * something laid out to transition FROM — toggling both in one step skips
 * straight to the end state and the drop never plays.
 */
export function showWhatsNew() {
  const scrim = $('whatsnew-scrim');
  ...
  // The close button, not the OK button: a screen reader user should land
  // somewhere they can act on immediately either way, and the corner button
  // is the one that is always in the same place regardless of how many
  // release items there are.
  $('whatsnew-close')?.focus();
}

/** Reverses showWhatsNew(). 450ms matches the panel's own transition time. */
export function hideWhatsNew() {
  const scrim = $('whatsnew-scrim');
  const panel = $('whatsnew');
  if (!scrim || !panel || panel.hidden) return;
